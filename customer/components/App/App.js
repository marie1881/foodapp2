...
var App = React.createClass({
    ...
    var _handleGetShopsDistance = (response, shop) => {
      var shopsWithDistance = this.state.shops.map(function(s) {
        if (s.place_id === shop.place_id) {
          return _.assign({}, s, {
            shopDistance: response.rows[0].elements[0].distance.text
          });
        } else {
          return s;
        }
  ...
  //     var expectedPickupTime = moment()
  //       .add(this.state.durationSeconds, "s")
  //       .format("LT");
  //   } else {
  ...
  _handlePreviousOrders: function() {
    request
      .get("/api/users/" + String(this.state.username) + "/orders/previous")
      .end((err, res) => {
        this.setState({
  ...
  _handleFavoriteOrders: function() {
    request
      .get("/api/users/" + String(this.state.username) + "/orders/favorites")
      .end((err, res) => {
        this.setState({
  ...
  _handleCCName: function(event) {
    var newPaymentInfo = _.assign({}, this.state.paymentInfo, {
        ...
        {!this.state.username ? (
          ...
          <div>
            <nav className="top-nav">
              ...
              <ul className={this.state.menuShow ? "menu-show" : "menu-hide"}>
                ...
                <Link
                  ...
                  onClick={this._handleUsernameRemove}
                >
                  <li
                    className="sign-out"
                    onClick={() => {
            ...
            {React.cloneElement(this.props.children, {
              ...
              toggleAdditionalInfoNotification: this
